...
- Loads and validates the ride via the shared `requireOwnedActiveOrFullRide(rideId, driverUserId, statusErrorMessage)` helper (`SELECT FOR UPDATE`, ownership check, ACTIVE/FULL check) — also used by `updateAvailableSeats`, `updateDepartureTime`, `updateRoute`, and `reannounceRide`, each passing their own status-error message. Extracted after this exact 6-line block existed nearly verbatim in all five methods; add new ride-mutation methods through this helper rather than re-inlining it.
- Validates: `newTotalSeats` differs from the current value (`InvalidRideStateException`, "No changes made"); `RideSeatLimits.MIN_TOTAL_SEATS (1) ≤ newTotalSeats ≤ RideSeatLimits.MAX_TOTAL_SEATS (8)` — the range check runs *before* the `reservedSeats` query so an out-of-range value never pays for it; `newTotalSeats ≥ reservedSeats` — `reservedSeats` comes from `bookingRepository.sumReservedSeats(rideId)` (in-app PENDING/CONFIRMED bookings only) — rejecting a shrink below what's already booked.
- `RideSeatLimits` (same package) is the single source of truth for the 1–8 bound, shared with `CreateRideRequest`'s `@Min`/`@Max` bean validation — the two can no longer drift apart.
- Sets both `totalSeats` and `availableSeats` (`newTotalSeats - reservedSeats`) together, then applies the shared `applySeatStatusFlip(ride, newAvailable)` helper (also used by `updateAvailableSeats`) for the FULL↔ACTIVE flip.
- Bidirectional — supports increasing and decreasing total capacity, not just the increase case that motivated it.
- Does **not** itself repost the group announcement or touch `announceCount`.
...
**`RideService.getReservedSeatsCount(rideId)`** — read-only delegate to `bookingRepository.sumReservedSeats(rideId)`. Exists so bot-side callers can show the driver the true reserved-seat floor before they type a new total. Do not derive this from `totalSeats - availableSeats` instead — that proxy overstates the floor whenever the driver has used `updateAvailableSeats` to manually show fewer available seats than the total actually allows (the "informal seat reduction" case documented under Re-Announce with Seat Count Edit in `carpool-bot/CLAUDE.md`), and understates the real minimum of 1 total seat when it computes to 0.

**`RideService.updateTotalSeatsAndReannounce(rideId, newTotalSeats, driverUserId)`** — calls `updateTotalSeats()` then `reannounceRide()` inside one `@Transactional` boundary. This is the method the bot flow calls, not the two steps separately: `updateTotalSeats` and `reannounceRide` calling each other as plain (non-proxied) method calls on `this` means Spring's self-invocation rule applies — their own `@Transactional` annotations become no-ops, and both steps execute inside the single transaction opened by `updateTotalSeatsAndReannounce`. This matters because `reannounceRide()` can reject the request on its own (the 10-announcement cap) — without the shared transaction, a rejected re-announce would still leave the seat-count change committed from the first call, silently out of sync with the group post the driver was told never happened.
...
`HubService.suggestHub()` prevents duplicate rows: if a hub with the same name+area already exists as ACTIVE it is returned as-is; if PENDING it is returned as-is (dedupe, no new row); if REJECTED it is flipped back to PENDING for review.

**Auto-approval by queue size:** once the PENDING queue reaches `AUTO_APPROVE_QUEUE_SIZE` (10), the suggestion that pushed it there triggers `suggestHub()` to auto-approve *every* pending hub via a shared `bulkApprovePending()` helper (same code-generation as `approveHub`) — the queue naturally resets to 0 since nothing is left pending. This skips manual admin review entirely once demand for new locations is high enough. `suggestHub()` carries a conditional `@CacheEvict` (`hubs`, `hub-search`, `condition = "#result.status().name() == 'ACTIVE'"`) so an auto-approval shows up in the live hub list immediately, same as a manual approval. `approveAllPendingHubs()` (the manual admin bulk-approve endpoint) shares the same `bulkApprovePending()` helper.

`bulkApprovePending()` reads the pending set via `HubRepository.findAllPendingForUpdate()` — a `PESSIMISTIC_WRITE` lock (`SELECT ... FOR UPDATE`), same pattern as `RideRepository.findByIdWithLock` (see "Booking: Pessimistic Locking" below). Without it, two `suggestHub()` calls both observing the queue at threshold could concurrently bulk-approve the same rows and race on `generateUniqueCode()`'s check-then-write against the unique `hubs.code` column. With the lock, the second transaction blocks until the first commits, then re-reads current (not stale) PENDING rows — so it only processes whatever is genuinely still pending, typically just its own hub. The plain `findAllPending()` (unlocked) stays in use for the read-only `getPendingHubs()` admin listing.
